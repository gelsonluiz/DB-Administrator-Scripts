...
Parameters
    @TableName(sysname): Optional parameter to add
...
Columns:
    index_action: Analysis recommendation on action to take on the index 
        ...
        REALIGN: Bookmark lookups on the index exceed the number of seeks on the table.  
                 Recommend investigating whether to move the clustered index to another index or add included columns to the indexes that are part of the bookmark lookups. 
...
BEGIN TRY
    ...
    SELECT @DB_ID = DB_ID()
        ,@ObjectID = OBJECT_ID(@TableName, DB_NAME(DB_ID()))
...
-- Obtain index meta data information
	...
	CREATE TABLE #IndexMeta (
     ...
     SET @SQL = N'SELECT
		...
		, included_columns = STUFF((
				...
				ORDER BY key_ordinal ASC
				FOR XML PATH('''')), 1, 2, '''') 
		, indexed_column_ids = (SELECT QUOTENAME(ic.column_id,''('')
				FROM '+@DatabaseName+'.sys.index_columns ic
				WHERE i.object_id = ic.object_id
				AND i.index_id = ic.index_id
				AND is_included_column = 0
	...
	SELECT IDENTITY(INT,1,1) AS row_id
        ...
        ,CAST(0 AS INT) AS missing_index_impact
        ,ius.user_seeks
        ,ius.user_scans
        ,ius.user_lookups
        ,ius.user_updates
    ...
    SET duplicate_indexes = STUFF((SELECT ', ' + index_name AS [data()]
            ...
            AND i.filter_definition = iibl.filter_definition
            AND i.index_id > iibl.index_id
            AND i.indexed_columns_ids = iibl.indexed_columns_ids
        ...
        ,overlapping_indexes = STUFF((SELECT ', ' + index_name AS [data()]
            ...
            AND i.filter_definition = iibl.filter_definition
            AND i.index_id <> iibl.index_id
            AND (i.indexed_columns_ids LIKE iibl.indexed_columns_ids + '%' 
                OR iibl.indexed_columns_ids LIKE i.indexed_columns_ids + '%')
            AND i.indexed_columns_ids <> iibl.indexed_columns_ids 
            FOR XML PATH('')), 1, 2, '')
    ...
    INSERT INTO #IndexStatistics
        (filegroup_name, schema_id, schema_name, object_id, table_name, index_name, type_desc, index_rank, indexed_columns)
    ...
    AS (
        SELECT row_id
            ,CASE WHEN user_lookups > user_seeks AND type_desc IN ('CLUSTERED', 'HEAP', 'UNIQUE CLUSTERED') THEN 'REALIGN'
                ...
                WHEN ROW_NUMBER() OVER (PARTITION BY table_name ORDER BY user_total desc, index_rank) > 10 THEN 'DROP' 
